...
class Auth
{
    ...
    public function check($name, $uid, $relation = 'or', $mode = 'url')
    {
        ...
        // 严格匹配节点 begin 下面代码非FastAdmin官方代码
        // @author fuyelk <fuyelk@fuyelk.com>

        // 非开发环境，则严格匹配节点
        if (!\think\Env::get('app.dev', false)) {
            // 检查节点及父节点是否均存在且正常
            $checkNode = AuthRule::alias('node')
                ->field('node.id')
                ->join('auth_rule p', 'p.id = node.pid')
                ->where('node.status', 'normal')
        ...
        // @author fuyelk <fuyelk@fuyelk.com>
        // 严格匹配节点 begin 上面代码非FastAdmin官方代码
